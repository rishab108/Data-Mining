...
object Rishab_Kumar_ItemBasedCF {
  def main(args: Array[String]): Unit = {
    ...
    for (test_data <- test_data_array) {
      ...
      if (item_averages_map.contains(item) && clusters_map.contains(user)) {
        ...
        if (similarities_matrix.contains(item))
          similarities_matrix_entry = similarities_matrix(item)
        else
          similarities_matrix += (item -> similarities_matrix_entry)

        // we get the items that user has rated and iterate over those items only
        val user_cluster = clusters_map(user)
        ...
        for (item_inner <- user_cluster) {
          ...
          if (similarities_matrix_entry.contains(item_inner))
            value = similarities_matrix_entry(item_inner)

          // when item pair is new and its similarity has not been calculated, we calculate it then
          else
            value = findSimilarity(item, ratings_matrix(item), item_inner, ratings_matrix(item_inner), item_averages_map)

          // if the similarity between two items is positive, then we add that item to its neighbors
          if (value > 0) {
            item_neighbors += (item_inner -> value)
          }
          // we store the calculated similarity in the similarity matrix for future use, irrespective of was it positive or negative
          similarities_matrix_entry += (item_inner -> value)
        }

        val item_neighbors_value_sorted = item_neighbors.toSeq.sortBy(_._2).reverse
        val item_neighbors_size = item_neighbors_value_sorted.length
        // iterate over top k neighbors and compute numerator and denominator
        for (i <- 0 until min(neighborhood, item_neighbors_size)) {

          rate += item_neighbors_value_sorted(i)._2 * (ratings_matrix(item_neighbors_value_sorted(i)._1)(user) - item_averages_map(item_neighbors_value_sorted(i)._1))
          // rate += item_neighbors_value_sorted(i)._2 * ratings_matrix(item_neighbors_value_sorted(i)._1)(user)
          similarity_sum += item_neighbors_value_sorted(i)._2
        }

        // check if the denominator in the rating formula is negative, then simply give item the rating as its average
        if (similarity_sum != 0) {
          prediction = item_averages_map(item) + (rate / similarity_sum)
          // prediction = rate / similarity_sum
          similarities_matrix += (item -> similarities_matrix_entry)
        } else {
      ...
      else if (item_averages_map.contains(item) && !clusters_map.contains(user)) {
    ...
    val outFileName = "src/main/Rishab_Kumar_ItemBasedCF.csv"
